...
from aerumentis.core.security import (
    AuthenticatedUser, UserRole, create_token_pair, decode_token, get_current_user,
)
from aerumentis.models.schemas import (
    LoginRequest, MessageResponse, RefreshRequest, RegisterRequest, TokenResponse, UserResponse,
)
...
async def register(request: RegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    if request.organization_name:
        org = Organization(
            id=str(uuid.uuid4()), name=request.organization_name,
...
async def get_me(current_user: AuthenticatedUser = Depends(get_current_user)):
    return UserResponse(id=current_user.user_id, email=current_user.email or "", full_name="",
                        role=current_user.role.value, org_id=current_user.org_id, is_active=True,
